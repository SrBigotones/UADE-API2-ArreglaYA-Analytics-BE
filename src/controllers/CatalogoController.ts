...
export class CatalogoController {
   ...
   * ENDPOINT DEPRECADO: Las zonas en solicitudes ya no se trackean
   * Retorna array vacío para mantener compatibilidad
   */
  public async getZonasSolicitudes(req: Request, res: Response): Promise<void> {
    try {
      logger.info(`⚠️ Deprecated endpoint called: /api/catalogo/zonas-solicitudes`);
      
      res.status(200).json({
        success: true,
        data: [],
        total: 0,
        message: 'Este endpoint está deprecado. Las zonas en solicitudes ya no se trackean.'
      });

    } catch (error) {
      logger.error('Error in deprecated zonas solicitudes endpoint:', error);
      res.status(500).json({
        success: false,
